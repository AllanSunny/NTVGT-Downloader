...
function downloadSong(song) {
    return new Promise((resolve, reject) => {
        ...
        video.on('info', function (info) {
            console.log(`Downloading audio for ${song.getName()}...`);
            video.pipe(fs.createWriteStream('./test/temp'));
        });

        video.on('error', reject);
        video.on('end', () => {
            console.log(`Downloaded audio for ${song.getName()}!`);
            resolve();
        });
    });
}
...
function trimSong(song) {
    return new Promise(resolve => {
        console.log(`Trimming audio for ${song.getName()}...`);
        childProcess.execFile('./util/FFMPEG/ffmpeg.exe', ['-hide_banner', '-y', '-loglevel', 'panic', '-i', './test/temp',
            ...
            () => {
            console.log(`Trimmed audio for ${song.getName()}!`);
            resolve();
        });
    });
}
...
function deleteTemp(song) {
    return new Promise(resolve => {
        console.log(`Deleting temporary file of ${song.getName()}...`);
